...
type Step = { key: string; label: string; options: string[] };
...
export function Configurator({
  ...
  const STEPS: Step[] = useMemo(
    () => [
      { key: "modelo", label: "Modelo", options: models.length ? models : ["Mocassim", "Scarpin", "Bota", "Tênis"] },
      { key: "couro", label: "Couro", options: couros.length ? couros : ["Liso", "Nobuck", "Verniz", "Camurça"] },
  ...
  return (
    <main className="min-h-screen bg-osso">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-5 py-10 sm:py-12">
        <h1 className="font-cormorant text-3xl sm:text-4xl text-ink">Monte seu modelo</h1>
              ...
              {current.options.map((o) => (
                <button
                  ...
                  className={`text-left px-4 py-3 rounded-[10px] border transition-colors ${
                    sel[current.key] === o ? "border-gold bg-gold/10" : "border-line hover:border-gold/50"
                  }`}
                >
                  {o}
                </button>
